...
const MarketplacePage = async () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      {/* Hero Section */}
      <section className="pt-32 pb-16 px-4 md:px-12 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-sky-400/20 via-slate-50 to-slate-50">
        <div className="max-w-7xl mx-auto text-center space-y-4">
          <h1 className="text-5xl md:text-7xl font-black tracking-tight text-slate-900 animate-in fade-in slide-in-from-bottom duration-700">
            Jelajahi <span className="text-sky-600">Ribuan Toko</span> <br/> Dalam Satu Genggaman.
          </h1>
          <p className="text-slate-500 text-lg md:text-xl max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom duration-1000">
      ...
      <section className="px-4 md:px-12 pb-24 max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
            <h2 className="text-2xl font-bold text-slate-800 flex items-center gap-x-2">
                <StoreIcon className="h-6 w-6 text-sky-600" />
                Daftar Toko Pilihan
            </h2>
            <div className="h-1 flex-1 bg-slate-200 mx-6 hidden md:block" />
            <span className="text-slate-500 font-medium">{stores.length} Toko Tersedia</span>
        </div>

        {stores.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-dashed border-slate-300">
            <StoreIcon className="h-16 w-16 text-slate-200 mb-4" />
            <p className="text-slate-500 text-lg">Belum ada toko yang terdaftar.</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {stores.map((store) => (
              <Link key={store.id} href={`/store/${store.id}`}>
                <Card className="group hover:shadow-2xl transition-all duration-300 border-none bg-white overflow-hidden shadow-sm">
                  <div className="h-40 bg-slate-100 relative">
                    {store.logoUrl ? (
                      ...
                      <div className="h-full w-full flex items-center justify-center bg-sky-50 text-sky-500">
                  ...
                  <CardHeader className="pb-2">
                    <CardTitle className="text-xl font-bold text-slate-800 line-clamp-1">{store.name}</CardTitle>
                  </CardHeader>
                  <CardContent className="pb-4">
                    <div className="flex gap-x-2 mb-4">
                        <span className="px-2 py-1 bg-slate-100 text-slate-600 text-xs rounded-md">Verified</span>
                        <span className="px-2 py-1 bg-sky-50 text-sky-600 text-xs rounded-md font-medium">Top Rated</span>
                    </div>
                  </CardContent>
                  <CardFooter className="pt-0 flex items-center justify-between">
                    <span className="text-xs text-slate-400">Bergabung {new Date(store.createdAt).toLocaleDateString()}</span>
